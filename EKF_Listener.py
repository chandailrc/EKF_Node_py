...
def x_handle(x):
    ...
    x[48] = (1/Ix)*(ms*hs**(x[15] + g*x[6]) - kroll*x[6] - croll*x[7])#x[-1+49]#; #(1/Ix)*(ms*hs*(-x[-1+16]+g*x[-1+7]) - kroll*x[-1+7] - croll*x[-1+8])#; #ms*g*h*x[-1+7] - ms*h*(x[-1+16] + x[-1+14]*x[-1+8]) + (x[-1+26]+x[-1+29]-x[-1+28]-x[-1+27])*d#;       # 49. rollA
...
def ekf(fstate,x,P,hmeas,z,Q,R):
    ...
    K=P12.dot(np.linalg.inv(H.dot(P12)+R));                       #Kalman filter gain
    x=x1+K.dot((np.array(z)-np.array(z1)));                            #state estimate
    P=P-K.dot(P12.T);                               #state covariance matrix
    #R=np.linalg.cholesky((H.dot(P12)+R))             #Cholesky factorization
    '''
    P2, L, U=scipy.linalg.lu((H.dot(P12)+R))             #Cholesky factorization
    R = P2
    print(R.shape)
...
for i in range(1,S):
    velX[i] = gt[i,0] - gt[i-1,0]
    velY[i] = gt[i, 1] - gt[i - 1, 1]
    velYaw[i] = gt[i, 2] - gt[i - 1, 2]
    velPitch[i] = gt[i, 3] - gt[i - 1, 3]
    velRoll[i] = gt[i, 4] - gt[i - 1, 4]
    velZ[i] = gt[i,5] - gt[i-1,5]
...
def listener():
    rospy.Subscriber("/mono_odometer/pose", PoseStamped, EKF, queue_size=1000)
    rospy.spin()



def EKF(p):
    global ax, f, x, P, Q, R, iterI, ptsVisual_X, ptsVisual_Y, ptsVisual_Z
    rospy.loginfo(rospy.get_caller_id() + '  I start %s', iterI)
    meas = [p.pose.position.z,  -p.pose.position.x, -p.pose.position.y,
    ...
    x_VO.append(float(p.pose.position.x))
    y_VO.append(float(p.pose.position.y))
    z_VO.append(float(p.pose.position.z))
    ...
    '''
    xh = open("VO_data_x.txt", "w")
    for item in x_VO:
        print>>xh, item
    xh.close()

    yh = open("VO_data_y.txt", "w")
    for item in y_VO:
        print>>yh, item
    yh.close()

    zh = open("VO_data_z.txt", "w")
    for item in y_VO:
        print>>zh, item
    zh.close()
    ...
    with open('/home/equaltrace/catkin_ws/src/viso2-indigo/viso2_ros/rata/Statdata.csv', 'wb') as fi:
        write = csv.writer(fi)
